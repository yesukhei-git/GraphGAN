...
def custom_wasserstein_gradient_penalty(
  ...
  with ops.name_scope(scope, 'custom_wasserstein_gradient_penalty',
    ...
    print(real_data.get_shape().as_list())
    real_data = project_3d_shape(tf.expand_dims(tf.transpose(real_data[...,0:3]),axis=0) , tf.expand_dims(tf.transpose(real_data[...,3:6]),axis=0), 
                        tf.expand_dims(tf.transpose(real_data[...,6:9]),axis=0), tf.expand_dims(tf.transpose(real_data[...,9]),axis=0), tf.expand_dims(tf.transpose(real_data[...,10]),axis=0))
    
    real_data.set_shape((real_data_shape[0],2))
    
#     #generated_data.numpy()
#     real_data = tf.where(real_data==0, 1.0, real_data )
#     generated_data = tf.where(tf.math.is_inf(generated_data), tf.zeros_like(generated_data), generated_data)
#     real_data = tf.where(tf.math.is_inf(real_data), tf.ones_like(real_data), real_data)
#     generated_data = tf.where(tf.math.is_nan(generated_data), tf.zeros_like(generated_data), generated_data)
#     real_data = tf.where(tf.math.is_nan(real_data), tf.ones_like(real_data), real_data)
